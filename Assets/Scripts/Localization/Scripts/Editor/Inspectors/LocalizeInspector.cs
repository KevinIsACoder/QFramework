...
namespace I2.Loc
{
	...
	public partial class LocalizeInspector : Editor
	{
		...
		SerializedProperty 	mProp_mTerm, mProp_mTermSecondary,
                            ...
                            mProp_CallbackEvent, mProp_Flip, mProp_Coordinate;
		...
		void OnEnable()
		{
			...
			mProp_Flip						= serializedObject.FindProperty("RectTranformFlip");
			mProp_Coordinate				= serializedObject.FindProperty("RectTranformCoordinate");
        ...
        public override void OnInspectorGUI()
		{
			...
			else
			{
				...
				if (eTermType.RectTransform == mLocalize.mLocalizeTarget.GetPrimaryTermType(mLocalize))
				{
					GUILayout.BeginVertical("Box");

					mProp_Flip.boolValue = GUILayout.Toggle(mProp_Flip.boolValue, new GUIContent("Flip", "如果是 true，RTL 时 则会翻转"));
					mProp_Coordinate.boolValue = GUILayout.Toggle(mProp_Coordinate.boolValue, new GUIContent("Coordinate", "如果是 true，RTL 时 则会改变x坐标"));
