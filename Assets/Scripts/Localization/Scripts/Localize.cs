...
namespace I2.Loc
{
    ...
    public partial class Localize : MonoBehaviour
    {
		...
		public bool RectTranformFlip = false;// RectTranform 是否用翻转
		public bool RectTranformCoordinate = true;// RectTranform 是否用改变x坐标
		...
		public void OnLocalize( bool Force = false )
		{
			...
			if (!hasCallback && string.IsNullOrEmpty (FinalTerm) && string.IsNullOrEmpty (FinalSecondaryTerm) && eTermType.RectTransform != mLocalizeTarget.GetPrimaryTermType(this))
				return;
			...
			if (!hasCallback && /*string.IsNullOrEmpty (MainTranslation)*/ string.IsNullOrEmpty(FinalTerm) && string.IsNullOrEmpty (SecondaryTranslation) && eTermType.RectTransform != mLocalizeTarget.GetPrimaryTermType(this))
				return;
			...
			if (eTermType.RectTransform == mLocalizeTarget.GetPrimaryTermType(this))
			{
				(mLocalizeTarget as LocalizeTarget_UnityUI_RectTransform).SetFlipAndCoordinate(RectTranformFlip, RectTranformCoordinate);
			}
